/* NULL · catalog.ts
   ...
   scripts/discover-games.mjs from the public stashes (truffled.lol, gn-math,
   the Ultimate Game Stash) — hundreds of games nobody had to type in. The
   discovered shelf is a build-time choice, not a runtime fetch: it is off in
   dev so the page stays honest about what this repo actually ships, and on
 ...
 *  it serves. Only merged into a real build — in dev the library is exactly
 *  what was put in by hand. */
function discovered(): Entry[] {
  if (import.meta.env.DEV) return [];
  const local = new Set(GAMES.map((g) => g.name.toLowerCase()));
  return DISCOVERED.filter((e) => !local.has(e.name.toLowerCase())).map((e) => ({
    ...e,
    labels: [...e.labels, "Remote"],
