...
/* The same words as the server's: the two roads must answer the same way, and
   a prompt that exists on only one of them is a tell. */
    ...
    const res = await fetch(URL, {
      ...
      body: JSON.stringify({
        ...
        messages: [
          ...
          ...history.slice(-8).map((m) => ({ role: m.role, content: m.content.slice(0, 6000) })),
        ],
      }),
    });
